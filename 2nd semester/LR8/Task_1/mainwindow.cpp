    ...
    , ui(new Ui::MainWindow)
{
    ...
    for (int i = 0; i < 16; ++i) {
        ...
        a[i].second = "";
        for (int j = 0; j < QRandomGenerator::global()->bounded(4) + 3; j++)
            a[i].second += QChar(QRandomGenerator::global()->bounded(26) + 'A');
    }

    tree = new BinarySearchTree<int, QString>(a);
...
void MainWindow::fillTreeWidgetFromBST(QTreeWidgetItem *item, BinarySearchTree<int, QString>::Node *node)
{
    if (node == nullptr)
        return;
...
{
    ui->resLAbel->setPlainText(QString::number(tree->FindLCA(ui->infoLineEdit->text().toInt(), ui->keySpinBox->value())->key));
}
